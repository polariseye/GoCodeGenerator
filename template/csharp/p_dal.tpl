...
namespace Moqikaka.GameServer.Inuyasha.DAL
{
    ...
    /// {{.TargetTable.Comment}}
    /// </summary>
    public class {{.TargetTable.TableName |EntityName}}DAL : GameBaseDAL
    {
        #region 属性
        ...
        {
            getAllCommand = String.Format("SELECT {0} FROM {{.TargetTable.TableName}} WHERE PlayerId = @PlayerId;", columns);
        ...
        public static DataTable GetList(Guid playerId)
        {
            MySqlParameter[] paramList = new MySqlParameter[]
			{
				new MySqlParameter(pnPlayerId, playerId)
			};

            return ExecuteDataTable(getAllCommand, paramList);
        }
