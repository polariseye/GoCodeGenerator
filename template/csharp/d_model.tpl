...
namespace Moqikaka.GameServer.Inuyasha.Model
{
	...
	/// {{.TargetTable.Comment}}
	/// </summary>
	public sealed class {{.TargetTable.TableName |EntityName}} : BaseDynamic
	{
        ...
        /// 构造函数
        /// </summary>
        /// <param name="dynamic">动态数据数据对象</param>{{range $index,$colItem:=.TargetTable.Columns}}
        /// <param name="{{FirstCharLower (FieldName $colItem.ColumnName)}}">{{$colItem.Comment}}</param>{{end}}
        public {{.TargetTable.TableName |EntityName}}(Dynamic dynamic{{range .TargetTable.Columns}}, {{FieldType .}} {{FirstCharLower (FieldName .ColumnName)}}{{end}})
            : base(dynamic)
        {
...
{{end}}
        ...
        /// 构造函数
        /// </summary>
        /// <param name="dynamic">动态数据数据对象</param>
        /// <param name="dr">System.Data.DataRow</param>
        public {{.TargetTable.TableName |EntityName}}(Dynamic dynamic, System.Data.DataRow dr)
            : base(dynamic)
        {
